...
export async function createLessonOutput(formData: LessonOutputFormData) {
  ...
  if (error) {
    ...
    // 23503 = foreign key violation: completed_by or assistant_id references a non-existent teacher
    if (code === '23503') {
      console.warn('[createLessonOutput] Foreign key violation, retrying without completed_by/assistant_id:', error.message);
      // Retry without foreign key fields
      payload.completed_by = null;
      payload.assistant_id = null;
      const { data: data2, error: error2 } = await supabase
        ...
        .single();
      if (error2) {
        throw new Error(formatSupabaseError(error2, 'create lesson output'));
      }
      return data2 as LessonOutput;
    }
