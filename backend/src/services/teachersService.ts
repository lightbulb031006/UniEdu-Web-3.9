...
export async function createTeacher(teacherData: Omit<Teacher, 'id'>) {
  // Generate ID if not provided (teachers.id is TEXT PRIMARY KEY with no default)
  const id = (teacherData as any).id || `T${Date.now()}${Math.random().toString(36).slice(2, 7).toUpperCase()}`;
  const insertData = { id, ...teacherData };
  ...
  if (accountHandle !== undefined || accountPassword !== undefined) {
    ...
    if (existingUsers && existingUsers.length > 0) {
      ...
      if (insertError) {
        ...
        if (code === '23505') {
          // Unique constraint violation - account already exists, try to update instead
          console.warn('[updateTeacher] User already exists (23505), trying update by link_id or email');
          const updatePayload: Record<string, unknown> = {};
          if (userUpdate.account_handle !== undefined) updatePayload.account_handle = userUpdate.account_handle;
          if (userUpdate.password) updatePayload.password = userUpdate.password;
          if (Object.keys(updatePayload).length > 0) {
            // Try update by link_id first
            const { error: retryError } = await supabase.from('users').update(updatePayload).eq('link_id', id);
            if (retryError) {
              console.error('[updateTeacher] Retry update also failed:', retryError.message);
            }
          }
        } else {
