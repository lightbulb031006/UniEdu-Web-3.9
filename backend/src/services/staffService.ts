...
export async function getStaffDetailData(staffId: string, month: string) {
  ...
  if (allRelevantClassIds.size === 0) {
    console.warn('[getStaffDetailData] Denormalized columns empty, falling back to class_teachers + sessions query');
    ...
    // Also get classes from sessions (taught in the past)
    ...
    if (sessionClassData) {
      const uniqueSessionClassIds = new Set(sessionClassData.map((s: any) => s.class_id));
      uniqueSessionClassIds.forEach((classId: string) => {
        allRelevantClassIds.add(classId);
        if (!taughtClassIds.includes(classId)) {
          taughtClassIds.push(classId);
        }
      });
    }
  }
