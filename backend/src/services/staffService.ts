...
export async function getStaffUnpaidAmounts(staffIds: string[]): Promise<{ totals: Record<string, number>; breakdown: Record<string, { classesAndWork: number; bonuses: number; total: number }> }> {
  ...
  const resultBreakdown: Record<string, { classesAndWork: number; bonuses: number; total: number }> = {};
  ...
  // Return breakdown: classesAndWork (sessions + workItems) vs bonuses
  // so frontend can apply deduction only to classesAndWork
  uncachedStaffIds.forEach((staffId) => {
    const classesAndWork = (unpaidSessions[staffId] || 0) + (unpaidWorkItems[staffId] || 0);
    const bonusesAmount = unpaidBonuses[staffId] || 0;
    result[staffId] = classesAndWork + bonusesAmount;
    resultBreakdown[staffId] = { classesAndWork, bonuses: bonusesAmount, total: classesAndWork + bonusesAmount };
  });
