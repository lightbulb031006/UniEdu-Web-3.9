...
/**
 * Generate JWT token
 * linkId: for teacher role, id of teachers record (staff profile) - used to enforce staff profile access
 */
export function generateToken(payload: { userId: string; role: string; email?: string; linkId?: string }, rememberMe: boolean = false): string {
...
export async function login(credentials: LoginCredentials): Promise<AuthResult> {
  ...
  // Generate tokens (include linkId for teachers so backend can enforce staff-profile access)
  const tokenPayload: { userId: string; role: string; email?: string; linkId?: string } = {
    userId: user.id,
    role: user.role,
    email: user.email,
  };
...
export async function refreshAccessToken(refreshToken: string): Promise<{ token: string; refreshToken: string }> {
  try {
    ...
    // Get user to verify they still exist and get their role/email/link_id
    const user = await getUserById(decoded.userId);

    // Generate new tokens (include linkId for teachers)
    const tokenPayload: { userId: string; role: string; email?: string; linkId?: string } = {
      userId: user.id,
      role: user.role,
      email: user.email,
    ...
    const newToken = generateToken(tokenPayload, false); // Use default expiration for refresh (not rememberMe)
