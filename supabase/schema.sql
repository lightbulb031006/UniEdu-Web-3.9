...
CREATE TABLE IF NOT EXISTS teachers (
    ...
    active_class_ids JSONB DEFAULT '[]'::jsonb, -- Denormalized: lớp đang dạy (từ class_teachers)
    taught_class_ids JSONB DEFAULT '[]'::jsonb, -- Denormalized: lớp từng dạy (từ sessions)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS classes (
    ...
    tuition_per_session INTEGER DEFAULT 0, -- Trợ cấp giáo viên (hệ số) mặc định
    scale_amount INTEGER DEFAULT 0,
    max_allowance_per_session INTEGER DEFAULT 0,
    student_tuition_per_session INTEGER DEFAULT 0, -- Học phí mỗi buổi (mặc định lớp)
    tuition_package_total INTEGER DEFAULT 0, -- Tổng tiền học phí gói (mặc định lớp)
    tuition_package_sessions INTEGER DEFAULT 0, -- Số buổi trong gói (mặc định lớp)
    schedule JSONB DEFAULT '[]'::jsonb, -- Array of {day, time}
    custom_teacher_allowances JSONB DEFAULT '{}'::jsonb, -- {teacherId: amount}
    teacher_ids JSONB DEFAULT '[]'::jsonb, -- Denormalized: array teacher ID (từ class_teachers)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS students (
    ...
    cskh_staff_id TEXT REFERENCES teachers(id) ON DELETE SET NULL,
    cskh_assigned_date DATE,
    cskh_unassigned_date DATE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS student_classes (
    ...
    student_fee_total INTEGER DEFAULT 0,
    student_fee_sessions INTEGER DEFAULT 0,
    student_tuition_per_session INTEGER DEFAULT 0,
    total_attended_sessions INTEGER DEFAULT 0,
    unpaid_sessions INTEGER DEFAULT 0,
    total_purchased_sessions INTEGER DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    tuition_fee INTEGER DEFAULT 0, -- Học phí buổi học (số tiền học sinh đóng cho buổi đó)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS attendance (
    ...
    student_id TEXT NOT NULL REFERENCES students(id) ON DELETE CASCADE,
    present BOOLEAN DEFAULT true, -- Deprecated: dùng status thay thế
    status VARCHAR(20) NOT NULL DEFAULT 'present' CHECK (status IN ('present', 'excused', 'absent')),
    remark TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS wallet_transactions (
    id TEXT PRIMARY KEY,
    student_id TEXT NOT NULL REFERENCES students(id) ON DELETE CASCADE,
    type TEXT NOT NULL CHECK (type IN ('topup', 'loan', 'advance', 'repayment', 'extend', 'refund')),
...
CREATE TABLE IF NOT EXISTS lesson_outputs (
    ...
    source TEXT, -- Nguồn bài (codeforces, Unicorns, LQDOJ, ...)
    original_link TEXT, -- Link gốc (URL nguồn bài)
    cost INTEGER DEFAULT 0,
    date DATE NOT NULL,
    status TEXT DEFAULT 'pending' CHECK (status IN ('paid', 'pending', 'deposit')),
    contest_uploaded TEXT,
...
-- ============================================
-- CLASS_SURVEYS TABLE (báo cáo khảo sát lớp)
-- ============================================
CREATE TABLE IF NOT EXISTS class_surveys (
    id TEXT PRIMARY KEY,
    class_id TEXT NOT NULL REFERENCES classes(id) ON DELETE CASCADE,
    test_number INTEGER NOT NULL,
    responsible_person_id TEXT REFERENCES teachers(id) ON DELETE SET NULL,
...
-- ============================================
-- STAFF_MONTHLY_STATS TABLE (cache thống kê tháng)
-- ============================================
CREATE TABLE IF NOT EXISTS staff_monthly_stats (
    id TEXT PRIMARY KEY,
    staff_id TEXT NOT NULL REFERENCES teachers(id) ON DELETE CASCADE,
    month TEXT NOT NULL,
    classes_total_month INTEGER DEFAULT 0,
    classes_total_paid INTEGER DEFAULT 0,
    classes_total_unpaid INTEGER DEFAULT 0,
    work_items_total_month INTEGER DEFAULT 0,
    work_items_total_paid INTEGER DEFAULT 0,
    work_items_total_unpaid INTEGER DEFAULT 0,
    bonuses_total_month INTEGER DEFAULT 0,
    bonuses_total_paid INTEGER DEFAULT 0,
    bonuses_total_unpaid INTEGER DEFAULT 0,
    total_month_all INTEGER DEFAULT 0,
    total_paid_all INTEGER DEFAULT 0,
    total_unpaid_all INTEGER DEFAULT 0,
    total_paid_all_time INTEGER DEFAULT 0,
...
-- ============================================
-- CSKH_PAYMENT_STATUS TABLE (trạng thái thanh toán lợi nhuận CSKH)
-- ============================================
...
-- ============================================
-- ACTION_HISTORY TABLE (lịch sử thao tác CRUD)
-- ============================================
...
ALTER TABLE class_surveys ENABLE ROW LEVEL SECURITY;
ALTER TABLE staff_monthly_stats ENABLE ROW LEVEL SECURITY;
ALTER TABLE cskh_payment_status ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "Allow all for authenticated users" ON class_surveys;
DROP POLICY IF EXISTS "Allow all for authenticated users" ON staff_monthly_stats;
DROP POLICY IF EXISTS "Allow all for authenticated users" ON cskh_payment_status;
DROP POLICY IF EXISTS "Allow all for authenticated users" ON action_history;
...
CREATE POLICY "Allow all for authenticated users" ON class_surveys FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow all for authenticated users" ON staff_monthly_stats FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow all for authenticated users" ON cskh_payment_status FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow all for authenticated users" ON action_history FOR ALL USING (true) WITH CHECK (true);
...
DROP TRIGGER IF EXISTS update_class_surveys_updated_at ON class_surveys;
DROP TRIGGER IF EXISTS update_staff_monthly_stats_updated_at ON staff_monthly_stats;
DROP TRIGGER IF EXISTS update_cskh_payment_status_updated_at ON cskh_payment_status;
...
CREATE TRIGGER update_class_surveys_updated_at BEFORE UPDATE ON class_surveys FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_staff_monthly_stats_updated_at BEFORE UPDATE ON staff_monthly_stats FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_cskh_payment_status_updated_at BEFORE UPDATE ON cskh_payment_status FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
